...
const AluminumPage = () => {
  ...
  const getDataCat = async () => {
    ...
    const res = await fetch(`/stone/api/category`);
    if (!res.ok) {
  ...
  const getData = async () => {
    ...
    const res = await fetch(`/stone/api?current=${meta.current}&pagesize=${meta.pageSize}`);
    if (!res.ok) {
      throw new Error(res.statusText);
    }
    const data = await res.json();
  ...
  const handleOnChange = async (page: number, pageSize: number) => {
    ...
    const res = await fetch(`/stone/api?current=${page}&pagesize=${pageSize}`);
    if (!res.ok) {
      throw new Error(res.statusText);
    }
    const data = await res.json();
