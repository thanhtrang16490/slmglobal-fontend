...
const SteelCasting = () => {
  ...
  const getData = async () => {
    ...
    const res = await fetch(`/steel-casting/api?current=${meta.current}&pagesize=${meta.pageSize}`);
    if (!res.ok) {
  ...
  const handleOnChange = async (page: number, pageSize: number) => {
    ...
    const res = await fetch(`/steel-casting/api?current=${page}&pagesize=${pageSize}`);
    if (!res.ok) {
      throw new Error(res.statusText);
    }
    const data = await res.json();
